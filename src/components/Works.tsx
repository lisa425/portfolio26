...
const NODE_3D = ORBITAL_RINGS.slice(0, 5).map((ring, i) =>
  getPointOnRing(i, ring.angle),
);
...
function Works({ isActive }: WorksProps) {
    ...
    const loop = () => {
      ...
      if (isActiveRef.current && entryDoneRef.current) {
        ...
        if (!isDragRef.current) {
          if (
            Math.abs(velRef.current.x) > 0.001 ||
            Math.abs(velRef.current.y) > 0.001
          ) {
        ...
        if (scene3dRef.current) {
          ...
          scene3dRef.current.style.setProperty(
            "--rx",
            `${rotRef.current.x}deg`,
          );
          scene3dRef.current.style.setProperty(
            "--ry",
            `${rotRef.current.y}deg`,
          );
        }
        ...
        ringHighlightSweeps.current.forEach((val, i) => {
          ...
          if (ringHighlightEls.current[i]) {
            ringHighlightEls.current[i]!.style.setProperty(
              "--draw-sweep",
              `${ringHighlightSweeps.current[i]}deg`,
            );
          }
        });
      }
  ...
  const handleNodeHover = useCallback((idx: number | null) => {
    ...
    ringHighlightEls.current.forEach((el, i) =>
  ...
  const calculateHover = useCallback(
    (clientX: number, clientY: number) => {
      ...
      if (candidates.length > 0) {
        candidates.sort((a, b) =>
          Math.abs(a.dist - b.dist) < 20 ? a.idx - b.idx : a.dist - b.dist,
        );
        ...
        const GAP = 30;
        const preferredSide =
          nodeScreenX < window.innerWidth / 2 ? "left" : "right";
        ...
        if (
          ...
          panelRefs.current[activePanelIdxRef.current]?.classList.remove(
            "active",
          );
        }
        ...
        if (panel) {
          ...
          if (side === "left" && nodeScreenX - GAP - panelW < 0) side = "right";
            ...
            side === "right" &&
            nodeScreenX + GAP + panelW > window.innerWidth
          )
            side = "left";
          ...
          panel.style.left =
            side === "right" ? `${nodeScreenX + GAP + 30}px` : "auto";
          panel.style.right =
            side === "left"
              ? `${window.innerWidth - nodeScreenX + GAP}px`
              : "auto";
  ...
  const handlePointerUp = useCallback(() => {
    ...
    if (sceneRef.current) {
      sceneRef.current.style.cursor =
        hoveredIndexRef.current !== null ? "pointer" : "grab";
    }
  }, [handleWorkClick]);
  ...
  useEffect(() => {
    if (isActive) {
      ...
      tl.fromTo(
        ".orbital-ring",
        { scale: 0 },
        { scale: 1, duration: 0.55, stagger: 0.07, ease: "power2.out" },
        0.1,
      );
  ...
  return (
    <div className="inner works__inner">
      ...
      {/* Telemetry Panel */}
      <div className="works-telemetry" ref={telemetryRef} />
      ...
      >
        ...
        {/* 3D world */}
        <div className="scene-3d" ref={scene3dRef}>
          {/* Base Rings */}
          ...
          {works.map((work, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span className="constellation-node__index">
                    {String(idx + 1).padStart(3, "0")}
                  </span>
      ...
      {createPortal(
        <div className={`works__detail ${isOpen ? "active" : ""}`}>
          <div className="works__detail-overlay" onClick={closeDetail} />
          <div className="works__detail-panel">
            ...
            <div className="panel-header">
              ...
              <button className="btn-close-panel" onClick={closeDetail}>
                [ X ] CLOSE
              </button>
            </div>
            ...
            {/* Panel Body */}
            {activeWork && (
              <div className="panel-body" key={activeWork.id}>
                <div className="panel-body__left">
                  <div className="panel-image-container">
                    ...
                    <div className="image-wrapper">
                      {activeWork.img && (
                        <img src={activeWork.img} alt="thumbnail" />
                      )}
                  ...
                  <div className="panel-meta">
                    <div className="meta-row">
                      ...
                      <span className="meta-val">
                        {CATEGORY_MAP[activeWork.category] ?? "secret"}
                      </span>
                    </div>
                    <div className="meta-row">
                    ...
                    <div className="meta-row">
                      ...
                      <span className="meta-val">{activeWork.stack}</span>
                    </div>
                  </div>

                  <div className="panel-title-wrapper">
                    <h2 className="panel-game text-display">
                      {activeWork.game}
                    </h2>
